  ...
  async _processEvent(event) {
    ...
    if (oc && oc.enabled && this.hooksUrl && this.hooksToken) {
      ...
      if (eventTypes.includes(type)) {
        try {
          ...
          // Structured message format — agent should process silently, not echo
          ...
          await this._postWithRetry(this.hooksUrl, {
            message,
            deliver: true,
    ...
    if (errors.length > 0) {
      ...
      if (attempts >= MAX_ATTEMPTS) {
        ...
        console.warn(`[delivery] Retrying event ${event.id} in ${backoffMs}ms (attempt ${attempts}/${MAX_ATTEMPTS})`);
