...
(function() {
  ...
  function loadState() {
    return new Promise(function(resolve) {
      ...
      // Check localStorage — resolve IMMEDIATELY if available (no server wait)
      try {
      ...
      // Fetch from REST (primary source of truth) — updates in background
      getJson('/api/pages/' + pageId + '/state')
        .then(function(result) {
          var serverData = result.data;
          if (serverData !== null) {
            // Server has state — update localStorage
            try {
              localStorage.setItem(LS_STATE_KEY, JSON.stringify({
                data: serverData,
                updatedAt: Date.now()
            ...
            if (!resolved) {
              ...
              // Already resolved with localStorage — fire onStateLoaded callbacks
              // so templates can update if server state differs
              stateLoadedCallbacks.forEach(function(cb) { try { cb(serverData); } catch(e) {} });
              stateLoadedCallbacks.length = 0;
            }
          } else if (!resolved) {
            resolved = true;
            resolve(localState); // null
          }
          // Fire remaining callbacks
          if (stateLoadedCallbacks.length > 0) {
